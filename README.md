...
Available arch options are rv128i, rv28im, rv128ima, rv128imac, rv128imafc, rv128imafdc. Available ABI options are llp128, llp128f, llp128d 128-bit support is experimental, known issues are:

-   soft floating point operations have erratic behaviour, support for 128-bit words is not done
-   hard floating points moves (fmv) are currently seen as illegal by qemu (soon to be corrected)
...
A typical way to configure and compile the 128-bit toolchain (with libs for all 128-bit configurations) is:

    $ mkdir build && cd build
    $ CFLAGS=-O0 CXXFLAGS=-O0 ../configure --enable-multilib --with-cmodel=medany --prefix=/tmp/rv128 
    $ make -j
