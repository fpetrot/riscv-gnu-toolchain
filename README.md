...
This is a fork of https://github.com/riscv-collab/riscv-gnu-toolchain done im May 2022.
It support, with bugs, 128-bit cross-development tools and allows to compile simple bare metal programs, including libfemto.
We never reached the point of testing with newlib because changes in binutils in August 2022 (C99 support) somehow broke many of our hacks and then we had no time to work on it seriously.

The 128up branch of the binutils is a rebase of these changes on the current binutils, done as time permits.
It starts to work again correctly, unit tests are now passing elf128, gas, ld, qemu, but more are necessary.
Gcc rebase will be done once the binutils are better validated.
...
# Original riscv-gnu-toolchain README
